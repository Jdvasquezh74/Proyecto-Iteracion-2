...
class SQLUsuario 
{
	 ...
	 */
	public long adicionarUsuario (PersistenceManager pm, long idUsuario, String nit, String ubicacion, String tipoJuridico, 
			int numeroDocumento, String tipoDocumento, String relacionComunidad) 
	{
		if (!(nit == null)) {
			Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaUsuario  () + "(Id, Nit, Ubicacion, TipoJuridico, RelacionComunidad) values (?, ?, ?, ?, ?)");
			q.setParameters(idUsuario, nit, ubicacion, tipoJuridico, relacionComunidad);
			return (long) q.executeUnique();      
		}
		else {
			Query q = pm.newQuery(SQL, "INSERT INTO " + pp.darTablaUsuario  () + "(Id, numeroDocumento, tipoDocumento, RelacionComunidad) values (?, ?, ?, ?)");
			q.setParameters(idUsuario, numeroDocumento, tipoDocumento, relacionComunidad);
			return (long) q.executeUnique();      
		}
	}
